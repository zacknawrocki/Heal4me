...
import {Avatar, Badge, Button, Input, List, message, Modal, Skeleton, Spin, Tabs} from 'antd';
import {ContactsOutlined, MessageOutlined, PlusCircleOutlined, SearchOutlined, TeamOutlined} from '@ant-design/icons';
import {
  reqAddGroup,
  reqDeleteFriend,
  reqDeleteGroup,
...
export default class Contact extends Component {
  ...
  render() {
    const {
      initLoading, list, spinning, showContact, friendId,
      visibleModal, titleModal, okText, confirmLoading, messageList
    } = this.state;
    const contact = {
        ...
        <Tabs defaultActiveKey="1">
          ...
          >
            <div className="searchBar">
              <Input placeholder="Search" style={{width: '200px', marginRight: '10px'}} value={this.state.inputValue}
            ...
            <Spin spinning={spinning} tip={'Friend list loading...'}>
              <List
                ...
                renderItem={item => (
                  <List.Item
                    actions={[<a key="list-loadmore-edit" onClick={this.toContact.bind(this, item)}>Chat</a>,
                      <a key="list-loadmore-more" onClick={this.deleteFriends.bind(this, item)}>Remove Friend</a>]}
                  >
                    <Skeleton avatar title={false} loading={item.loading} active>
                      <List.Item.Meta
                        avatar={
                          <Avatar
                            src={item.friend_info ? item.friend_info.avatar : "https://www.pngkey.com/png/detail/46-461081_cartoon-of-group-of-people.png"}/>
                        }
                      ...
                      />
                      <div>Time
                        Registered:{moment(item.friend_info ? item.friend_info.date : '').format('YYYY-MM-DD hh:ss')}</div>
                    </Skeleton>
                  </List.Item>
                )}
              />
          ...
          >
            ...
            <Spin spinning={spinning} tip={'Group list loading...'}>
              <List
                ...
                renderItem={item => (
                  <List.Item
                    actions={[<a key="list-loadmore-edit" onClick={this.toContactGroup.bind(this, item)}>Chat</a>,
                      <a key="list-loadmore-more" onClick={this.deleteGroup.bind(this, item._id)}>Delete Group</a>]}
                  >
                    <Skeleton avatar title={false} loading={item.loading} active>
                      <List.Item.Meta
                        avatar={
                          <Avatar
                            src={item.avatar || "https://www.pngkey.com/png/detail/46-461081_cartoon-of-group-of-people.png"}/>
                        }
                        title={<span style={{fontSize: '18px'}}>{item.roomName}</span>}
                        description={
                          <span>Number of members: {this.state.groupList.length > 0 ? item.roomMember.length : 0}</span>}
                      />
                  ...
                  if (item.friend_id === store.getState().auth.user._id && item.isAllowed !== 0) {
                    return (<List.Item
                      actions={item.friend_id === store.getState().auth.user._id && item.isAllowed === 0 ?
                        [<a key="list-loadmore-edit" onClick={this.agreeAdd.bind(this, item)}>Accept</a>,
                          <a key="list-loadmore-more" onClick={this.rejectAdd.bind(this, item)}>Decline</a>] : [<a
                          key="list-loadmore-more" onClick={this.confirmAdd.bind(this, item)}>Confirm</a>]}
                    >
                      <Skeleton avatar title={false} loading={item.loading} active>
                        <List.Item.Meta
                          avatar={
                          ...
                          title={<span
                            style={{fontSize: '18px'}}>{item.type === 1 ? 'Friend Request' : 'Instant Message'}</span>}
                          description={
                            <span>{item.isAllowed === 1 ? 'Content: you are friends now!' : 'The request has been declined'}</span>}
                        />
                    ...
                    return (<List.Item
                      actions={item.friend_id === store.getState().auth.user._id && item.isAllowed === 0 ?
                        [<a key="list-loadmore-edit" onClick={this.agreeAdd.bind(this, item)}>Accept</a>,
                          <a key="list-loadmore-more" onClick={this.rejectAdd.bind(this, item)}>Decline</a>] : [<a
                          key="list-loadmore-more" onClick={this.confirmAdd.bind(this, item)}>Remove</a>]}
                    >
                      <Skeleton avatar title={false} loading={item.loading} active>
                        <List.Item.Meta
                          avatar={
